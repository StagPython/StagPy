...
parser = argparse.ArgumentParser(description='Read and process StagYY profile data')
...
def plotprofiles(quant, *vartuple):
    ...
    '''A kwarg should be used for different options, e.g. whether densities of total values are plotted'''
    if quant == 'Grid':
        fig, ax = plt.subplots(2, sharex=True)
    else:
        fig = plt.figure()
    for step in range(istart-1, ilast, istep):
        ...
        if quant == 'Grid':
            ...
            # keep the same color for the same timestep,
            # change the pattern between the elements in the vartuple
            for j in vartuple:
                if jj == 0:
                    pplot=ppl.plot(data[i0:i1, j], data[i0:i1, 0], linewidth=LWDTH,
                            label=r'$t=%.2e$' % (tsteps[step-1, 2])) # needs a shorter format !
                    col = pplot[0].get_color()
                    if ((quant == 'Concentration' or quant == 'Temperature') and
                    ...
                    if jj == 1:
                        ...
                        LSTYLE = '-.'
                    print jj,LSTYLE
                    ppl.plot(data[i0:i1, j], data[i0:i1, 0], c=col, ls = LSTYLE, linewidth=LWDTH)
                    jj = jj+1
            plt.ylim([-0.1, 1.1])
    ...
    plt.savefig(quant.replace(' ', '_')+".pdf", format='PDF')

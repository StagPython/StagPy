...
class ReadStagyyData:
    ...
    def __init__(self, fpath, fname, par_type, ti_fn):
        ...
        self.fullname = fpath + fname + '_' + par_type + \
            number_string  # name of the file to read
    ...
    def catch_header(self):
        ...
        self.nthtot = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        # total number of values in the longitude direction
        self.nphtot = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        # total number of values in the radius direction
        self.nrtot = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        # of blocks, 2 for yinyan
        self.nblocks = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        ...
        self.nnth = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        # in the th,ph,r and b directions
        self.nnph = struct.unpack('i', fid.read(4))[0]
        self.byte_offset = self.byte_offset + 4
        ...
        self.rg = struct.unpack(
            'f' * self.nr2, fid.read(4 * self.nr2))  # r-coordinates
        ...
        self.th_coord = struct.unpack(
            'f' * self.nthtot, fid.read(4 * self.nthtot))  # th-coordinates
        self.byte_offset = self.byte_offset + 4 * self.nthtot
        self.ph_coord = struct.unpack(
            'f' * self.nphtot, fid.read(4 * self.nphtot))  # ph-coordinates
        self.byte_offset = self.byte_offset + 4 * self.nphtot
        self.r_coord = struct.unpack(
            'f' * self.nrtot, fid.read(4 * self.nrtot))  # r-coordinates
            ...
            for irc in np.arange(self.nnr):
                for iphc in np.arange(self.nnph):
                    for ithc in np.arange(self.nnth):
                        ...
                        fileContent = struct.unpack(
                            'f' * npi, fid.read(4 * npi))
